window.onload = function() {
	...
	// let pixiContext = pixiCanvas.getContext('2d');
	...
	if (AudioContext) {
		...
		function renderFrame() {
			...
			// Pixi Canvas
			// paintBackground(pixiContext, "black");
			...
			for (var i = 0; i < num_bars; i++) {
				...
				context2.arc(canvas2.width/2, canvas2.height/2, radius, startAngle, endAngle);
				context2.stroke();
			}
			...
			// CANVAS 6 //
			const num_bars6 = 15;
			bin_size = Math.floor(frequencyData.length / num_bars6);

			paintBackground(context6, "black");

			for (var i = 0; i < num_bars6; i++) {
				let sum = 0;
				for (var j = 0; j < bin_size; j++) {
				...
				let average = sum / bin_size;
				let bar_width = (canvas2.width - 140) / (num_bars6 * 3);
				...
				let endAngle = Math.PI + (Math.PI * j) / 2; // End point on circle
				let radius = canvas2.width/2 - i*bar_width - 100 + scaled_average > 0 ? canvas2.width/2 - i*bar_width - 100 + scaled_average : 0;
				context6.arc(canvas2.width/2, canvas2.height/2, radius, startAngle, endAngle, false);

				if (scaled_average > 0) {
					context6.lineWidth = num_bars6 - i > 1 ? num_bars6 - i : 1;
					context6.strokeStyle = "#"+((1<<24)*Math.random()|0).toString(16);
					context6.stroke();
				} else {
					context6.lineWidth = 0;
				}
			}
		}
